<?php
    $SQL = "localhost:3306";
    function auth($cookie) { /// 任何网页应该先调用这个函数进行身份验证，返回值为 {0: 未登录, i (i>0): 已登录, -1: Server Error}，参数为 SESSIONID
        global $SQL;
        if(!$cookie) return 0;
        ...
        if(mysqli_num_rows($user) == 0) return 0;
        $array = mysqli_fetch_array($user);
        $expire = time() + 36000;
        if($array['expire'] < time()) return 0;
        setcookie("SESSIONID", $cookie, $expire); /// 更新 expire time
        $user_id = $array['userid'];
        mysqli_query($conn, "UPDATE auth SET expire=$expire WHERE userid=$user_id");
        mysqli_close($conn);
        return $user_id; /// 返回用户编号以便继续处理
    }
    function login($user) { /// 处理登录请求，返回值为 {-1: Server Error, 0: Success}。此函数默认用户存在，参数为userid
        ...
        if(mysqli_num_rows($isloggedin) != 0)
